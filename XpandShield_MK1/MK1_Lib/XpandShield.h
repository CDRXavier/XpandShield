...
constexpr uint8_t A_BUTTON =0x20;
constexpr uint8_t B_BUTTON =0x21;



constexpr uint8_t RED_LED  = 10;
constexpr uint8_t GREEN_LED =11;
constexpr uint8_t BLUE_LED  =9;
...
constexpr uint8_t OLED_DC   =  4;  //4 is used as chip select for alternate spi mode. how about 9?
...
//#define SPI_TRANSACTION_START SPI.beginTransaction(spiSet) ///< Pre-SPI
...
(void)
{
  if (digitalRead(A0) == LOW) {
    XpandShield::setRGBled(255, 255, 255);
    while (true);
  }
  XpandShield::setRGBled(240, 0, 0);
  delay(500);
  XpandShield::setRGBled(0, 240, 0);
  delay(500);
  XpandShield::setRGBled(0, 0, 240);
  delay(500);
  XpandShield::setRGBled(0, 0, 0);
...
(void)
{
  if ((!XpandShield::buffer) && !(XpandShield::buffer = (uint8_t *)malloc(XpandShield::WIDTH * ((XpandShield::HEIGHT + 7) / 8))))
    return;
